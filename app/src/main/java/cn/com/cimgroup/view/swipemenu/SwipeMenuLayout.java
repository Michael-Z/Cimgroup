...
public class SwipeMenuLayout extends FrameLayout {
	...
	private RelativeLayout message_without_detail;
	...
	public SwipeMenuLayout(View contentView, SwipeMenuView menuView,
		...
		if(mContentView != null){
			message_without_detail = (RelativeLayout) mContentView.findViewById(R.id.message_without_detail);
		}
	}
	...
	private void init() {
		...
		if (mContentView.getId() < 1) {
			mContentView.setId(R.id.swipe_menu_content_view_id);
		}

		mMenuView.setId(R.id.swipe_menu_view_id);
...
//				mContentView.getWidth() - dis, getMeasuredHeight());
		//修改侧滑区域   详情部分不侧滑
		//注意--只修改这里会出现侧滑闪烁现象  需要同时修改smoothCloseMenu（）和smoothOpenMenu（）方法 替换startScroll（）中的参数
		message_without_detail.layout(-dis, message_without_detail.getTop(),
				message_without_detail.getWidth() - dis, message_without_detail.getMeasuredHeight());
	...
	public void smoothCloseMenu() {
		state = STATE_CLOSE;
		mBaseX = -message_without_detail.getLeft();
	...
	public void smoothOpenMenu() {
		state = STATE_OPEN;
		mOpenScroller.startScroll(-message_without_detail.getLeft(), 0,
