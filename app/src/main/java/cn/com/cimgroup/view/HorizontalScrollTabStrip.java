...
public class HorizontalScrollTabStrip extends HorizontalScrollView {
	...
	/** 激活时颜色 */
	private Integer mActiveColor = 0;

	/** 默认颜色 */
	private Integer mDefaultColor = 0;
	...
	public HorizontalScrollTabStrip(Context context, AttributeSet attrs,
		...
		mActiveColor = context.getResources().getColor(R.color.color_red);
		mDefaultColor = context.getResources().getColor(R.color.color_gray_indicator);
	...
	private void resetLayoutParams() {
		mDefaultTagWidth = (int) (mScreenWidth / (mDefaultShowTagCount + 0.6));
	 ...
	 *            标题
	 */
	public void setTags(List<String> titles, boolean isChange) {
		mTitles.clear();
	 ...
	 * @param position
	 */
	public void changeActiviveTagText(int position) {
	 ...
	 * @param position
	 */
	public void changeActiveTag(int position) {
		isScrool = false;
		int size = tabsContainer.getChildCount();
		if (size == 0) {
			return;
		...
		for (int i = 0; i < size; i++) {
			...
			if (i == position) {
				...
				if (isFirstSetting) {
					...
					location[0] = mDefaultTagWidth * position;
				}
				tv.setTextColor(mActiveColor);
			} else
				tv.setTextColor(mDefaultColor);
		...
		if (location[0] < 0) {
			...
			if (mTagIndex < tabsContainer.getChildCount() - 1) {
		...
		if (null != mListener) {
			if (motionLength == 0) {
				mListener.changeLine(location[0], mTagIndex, true);
			} else {
				mListener.changeLine(location[0] - motionLength, mTagIndex,
						true);
			}
		}
	...
	public Handler mHandler = new Handler() {
		public void handleMessage(android.os.Message msg) {
			if (motionTimes > 0) {
				...
				isScrool = true;
				if (null != mListener) {
					mListener.updateDatas(mTagIndex);
				}
			}
		}
	};
	...
	public interface TagChangeListener {
		 ...
		 *            是否是点击触发
		 */
		void changeLine(int location_x, int position, boolean isClick);
	...
	/** 滚动控件内容 */
	public void tagScrollBy(int offset_x) {
		this.scrollBy(offset_x, 0);
	...
	protected void onScrollChanged(int l, int t, int oldl, int oldt) {
		...
		if (isScrool) {
			...
			if (null != mListener) {
				mListener.changeLine(location[0], mTagIndex, false);
			}
		}
