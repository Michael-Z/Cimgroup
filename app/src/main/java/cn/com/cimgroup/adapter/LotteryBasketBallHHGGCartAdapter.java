 ...
 * @Date:2016年1月25日
 */
public class LotteryBasketBallHHGGCartAdapter extends BaseLotteryBidCartAdapter implements
	...
	public LotteryBasketBallHHGGCartAdapter(Context context, MatchSearchType searchType, FootballLotteryHHGGUtil footballLotteryHHGGUtil) {
		super(context);
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		// 胜平负 按钮选择的结果集
		final List<Integer> selectIndex1 = selectWay.get(FootballLotteryConstants.L307) == null ? new ArrayList<Integer>() : selectWay.get(FootballLotteryConstants.L307);
		// 让球胜平负 按钮选择的结果集
		final List<Integer> selectIndex2 = selectWay.get(FootballLotteryConstants.L306) == null ? new ArrayList<Integer>() : selectWay.get(FootballLotteryConstants.L306);
		// 胜分差
		final List<Integer> selectIndex3 = selectWay.get(FootballLotteryConstants.L308) == null ? new ArrayList<Integer>() : selectWay.get(FootballLotteryConstants.L308);
		// 大小分
		final List<Integer> selectIndex4 = selectWay.get(FootballLotteryConstants.L309) == null ? new ArrayList<Integer>() : selectWay.get(FootballLotteryConstants.L309);
		
		StringBuilder sb = new StringBuilder();
		if(sfSp.indexOf("@") != -1){
			String[] spArr = sfSp.split("\\@");
			for (int i = 0; i < selectIndex1.size(); i++) {
				sb.append(spArr[selectIndex1.get(i)].split("\\_")[0] + " ");
			}
		}
		
		if(rsfSp.indexOf("@") != -1){
			String[] rspfArr = rsfSp.split("\\@");
			for (int i = 0; i < selectIndex2.size(); i++) {
				String text = rspfArr[selectIndex2.get(i)].split("\\_")[0];
				if (text.indexOf("|") != -1) {
					String[] rArr = text.split("\\|");
					sb.append("让" + rArr[1] + " ");
				} else {
					sb.append("让" + text + " ");
				}
				
//				String[] rArr = rspfArr[selectIndex2.get(i)].split("\\_")[0].split("\\|");
//				sb.append("让" + rArr[1] + " ");
			}
		}
		
		if(sfcSp.indexOf("@") != -1){
			String[] bqcArr = sfcSp.split("\\@");
