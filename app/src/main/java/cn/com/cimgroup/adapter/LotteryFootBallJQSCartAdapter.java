...
public class LotteryFootBallJQSCartAdapter extends BaseLotteryBidOnlyCartAdapter implements
		OnClickListener {
	...
	public LotteryFootBallJQSCartAdapter(Context context, MatchSearchType searchType, FootballLotteryUtil footballLotteryUtil) {
		super(context);
...
//		int dataPosition = super.mSelectItemKeys.get(position);
		...
		String sp = FootballLotteryTools.getMatchSp(match.getMatchAgainstSpInfoList(), GlobalConstants.TC_JZJQS, mSearchType);
		// 获取该条在全部比赛中得索引位置
			...
			if (mHolder.mFoolballNumbersView[i].getChildCount() == 2) {
				...
				if (spTextView != null) {
					if(sp.indexOf("@") != -1){
						...
						// 将matchNo /index 存储到mFoolballNumbersView中 以便标识该控件的位置
