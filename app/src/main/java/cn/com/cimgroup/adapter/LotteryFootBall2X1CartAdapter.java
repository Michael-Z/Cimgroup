...
public class LotteryFootBall2X1CartAdapter extends BaseLotteryBidOnlyCartAdapter implements
		OnClickListener {
	...
	public LotteryFootBall2X1CartAdapter(Context context, MatchSearchType searchType, FootballLotteryUtil footballLotteryUtil) {
		super(context);
...
//		int dataPosition = super.mSelectItemKeys.get(position);
		...
		mHolder.mBigLayoutView.setTag(new Integer[] { position, 0 });
		mHolder.mSmallLayoutView.setTag(new Integer[] { position, 1 });
...
//								return;
				...
				if (dans.size() > 0) {
					...
					if (dans.contains(Integer.valueOf(position))) {
						int index1 = dans.indexOf(position);
						for (int i = 0; i < dans.size(); i++) {
							if (i > index1) {
								int value = dans.get(i);
								dans.remove(i);
