 ...
 * @Date:2015年12月31日
 */
public class LotteryOldFootBallBQCAdapter extends BaseLotteryBidOnlyAdapter
		implements OnClickListener {

	public LotteryOldFootBallBQCAdapter(Context context) {
		super(context);
	}
	...
	public View getGroupView(int groupPosition, boolean isExpanded,
		...
		sb.append(mContext.getResources().getString(
				R.string.record_tz_detail_qi, infos.get(1).getIssue()));
		sb.append(DateUtil.getTimeInMillisToStr(infos.get(1).getEndTime() + "")
				+ "截止");
	...
	public View getChildView(int groupPosition, int childPosition,
		...
		if (matchTypeLayout != null && matchTypeLayout.getChildCount() == 0) {
			LinearLayout view = (LinearLayout) View.inflate(mContext,
					R.layout.include_oldfootball_bqc_select, null);
			matchTypeLayout.addView(view, new LayoutParams(
					LayoutParams.MATCH_PARENT, LayoutParams.MATCH_PARENT));
			mHolder.mFoolballNumbersView = new LinearLayout[] {
					((LinearLayout) view
							.findViewById(R.id.txtView_footballlottery_bqc_select1)),
					((LinearLayout) view
							.findViewById(R.id.txtView_footballlottery_bqc_select2)),
					((LinearLayout) view
							.findViewById(R.id.txtView_footballlottery_bqc_select3)),
					((LinearLayout) view
							.findViewById(R.id.txtView_footballlottery_bqc_select4)),
					((LinearLayout) view
							.findViewById(R.id.txtView_footballlottery_bqc_select5)),
					((LinearLayout) view
							.findViewById(R.id.txtView_footballlottery_bqc_select6)) };
		}
		// 获取该条数据在整个数据集中得索引
		...
		for (int i = 0; i < mHolder.mFoolballNumbersView.length; i++) {
			// 赋值给每个进球个数view
			mHolder.mFoolballNumbersView[i].setTag(new Object[] { dataPosition,
					i });
	...
	public void onClick(View v) {
		switch (v.getId()) {
		case R.id.txtView_footballlottery_bqc_select1:
		case R.id.txtView_footballlottery_bqc_select2:
		case R.id.txtView_footballlottery_bqc_select3:
		case R.id.txtView_footballlottery_bqc_select4:
		case R.id.txtView_footballlottery_bqc_select5:
		case R.id.txtView_footballlottery_bqc_select6:
