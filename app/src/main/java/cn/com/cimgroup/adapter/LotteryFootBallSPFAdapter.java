 ...
 * @Date:2015年11月16日
 */
public class LotteryFootBallSPFAdapter extends BaseLotteryBidAdapter implements
		OnClickListener {

	/**
	 * 用于存储选择投注选项的索引 key=所选item在数据集中得索引位置 value=所选item的投注方式索引<Map<Integer,
	 * List<Integer>> key=投注方式 value=投注方式索引
	 **/
	...
	public LotteryFootBallSPFAdapter(Context context, MatchSearchType searchType) {
		super(context);
	 ...
	 * @date:2016年1月25日
	 */
	public void setSelectItemIndexs(
	...
	public View getChildView(int groupPosition, int childPosition,
		...
		if (matchTypeLayout != null && matchTypeLayout.getChildCount() == 0) {
			LinearLayout view = (LinearLayout) View.inflate(mContext,
					R.layout.include_footballlottery_spf_select, null);
			matchTypeLayout.addView(view, new LayoutParams(
					LayoutParams.MATCH_PARENT, LayoutParams.MATCH_PARENT));
			mHolder.mFootballSPFViews = new LinearLayout[] {
					(LinearLayout) view
							.findViewById(R.id.txtView_footballlottery_hhgg_spf_0),
					(LinearLayout) view
							.findViewById(R.id.txtView_footballlottery_hhgg_spf_1),
					(LinearLayout) view
							.findViewById(R.id.txtView_footballlottery_hhgg_spf_2) };
			mHolder.mFootballrSPFViews = new LinearLayout[] {
					(LinearLayout) view
							.findViewById(R.id.txtView_footballlottery_hhgg_sspf_0),
					(LinearLayout) view
							.findViewById(R.id.txtView_footballlottery_hhgg_sspf_1),
					(LinearLayout) view
							.findViewById(R.id.txtView_footballlottery_hhgg_sspf_2) };
			((TextView) view.findViewById(R.id.txtView_football_item_select))
					.setVisibility(View.GONE);
			mHolder.spfStop = (RelativeLayout) view
					.findViewById(R.id.txtView_footballlottery_hhgg_spf_stop);
			mHolder.rspfStop = (RelativeLayout) view
					.findViewById(R.id.txtView_footballlottery_hhgg_sspf_stop);
			mHolder.rspfText = (TextView) view
					.findViewById(R.id.txtView_footballlottery_hhgg_allow1);
		}
		...
		Map<Integer, List<Integer>> selectWay = mSelectItemIndexs
				.get(dataPosition);
		// 选择的投注方式索引集合 胜平负
		List<Integer> selectIndex1 = selectWay != null ? (selectWay
				.containsKey(FootballLotteryConstants.L320) ? selectWay
				.get(FootballLotteryConstants.L320) : null) : null;
		// 选择的投注方式索引集合 让球胜平负
		List<Integer> selectIndex2 = selectWay != null ? (selectWay
				.containsKey(FootballLotteryConstants.L301) ? selectWay
				.get(FootballLotteryConstants.L301) : null) : null;
		// 将比率显示出来
		...
		if (sp == null) {
			...
			if (sp.length > 0) {
				...
				int spIndex = 0;
				for (String i : spList) {
					TextView spTextView = (TextView) mHolder.mFootballSPFViews[spIndex]
							.getChildAt(1);
					if (!StringUtil.isEmpty(i)) {
						mHolder.spfStop.setVisibility(View.GONE);
						spTextView.setText(i);
						mHolder.mFootballSPFViews[spIndex].setTag(new Object[] {
								dataPosition, FootballLotteryConstants.L320,
								spIndex });
						mHolder.mFootballSPFViews[spIndex]
		...
		if (rsp == null) {
			...
			if (rsp.length > 0) {
				...
				for (String i : rspList) {
					TextView spTextView = (TextView) mHolder.mFootballrSPFViews[rspIndex]
							.getChildAt(1);
					if (!StringUtil.isEmpty(i)) {
						mHolder.rspfStop.setVisibility(View.GONE);
						spTextView.setText(i);
						mHolder.mFootballrSPFViews[rspIndex]
								.setTag(new Object[] { dataPosition,
										FootballLotteryConstants.L301, rspIndex });
						mHolder.mFootballrSPFViews[rspIndex]
								.setOnClickListener(this);
	...
	public void onClick(View v) {
		...
		default:
			...
			if (obj != null && obj.length == 3) {
				...
				if (!mSelectItemIndexs.containsKey(dataPosition1)) {
					if (mSelectItemIndexs.size() >= GlobalConstants.MUSTNUM) {
						ToastUtil.shortToast(mContext, mContext.getResources()
								.getString(R.string.lottery_football_must));
					} else {
						mSelectItemIndexs.put(dataPosition1,
				...
				((LotteryFootballActivity) mContext)
