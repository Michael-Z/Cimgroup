...
import cn.com.cimgroup.adapter.LotteryFootBallBQCAdapter;
 ...
 * @Date:2016年1月25日
 */
public class LotteryFootballBQCFrament extends BaseLotteryBidFrament {

	// 半全场Adapter
	private LotteryFootBallBQCAdapter mAdapter = null;
	
	List<String> spiltTimes = new ArrayList<String>();
	
	List<List<Match>> mMatchs = new ArrayList<List<Match>>();

	private int mlotoId = FootballLotteryConstants.L304;
	...
	public void initData() {
		mAdapter = new LotteryFootBallBQCAdapter(mFragmentActivity, mSearchType);
	...
	private CallBack mCallBack = new CallBack() {
		 ...
		 * 足彩半全场加载成功
		 */
		public void getFootBallMatchsBQCSuccess(final FootBallMatch info) {
			//将引用传递中得值重新赋值给变量
		 ...
		 * 足彩半全场获取失败
		 */
		public void getFootBallMatchsBQCFailure(final String error) {
			if (mFragmentActivity != null) {
				mFragmentActivity.runOnUiThread(new Runnable() {
	...
	protected void loadData(int page) {
		// TODO Auto-generated method stub
		((LotteryFootballActivity)getActivity()).addData(3);
	}
