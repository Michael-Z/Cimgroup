 ...
 * @Date:2015年11月16日
 */
public class LotteryFootballSPFrament extends BaseLotteryBidFrament {

	private LotteryFootBallSPFAdapter mAdapter = null;
	
	List<String> spiltTimes = new ArrayList<String>();
	
	List<List<Match>> mMatchs = new ArrayList<List<Match>>();
	
	List<List<Match>> mSingleMatchs = new ArrayList<List<Match>>();
	
	List<List<LotterySp>> sps = new ArrayList<List<LotterySp>>();

	private int mlotoId = FootballLotteryConstants.L320;
	...
	public void initData() {
		mAdapter = new LotteryFootBallSPFAdapter(mFragmentActivity, mSearchType);
	...
	protected void loadData(int page) {
		((LotteryFootballActivity)getActivity()).addData(0);
	}
	...
	private CallBack mCallBack = new CallBack() {
		public void getFootBallMatchsInfoSPFSuccess(final FootBallMatch info) {
			//将引用传递中得值重新赋值给变量
								...
								case SINGLE:
									for (int i = 0; i < matchs.size(); i++) {
										...
										for (int j = 0; j < match.getMatchs().size(); j++) {
											...
											List<MatchAgainstSpInfo> sps = m.getMatchAgainstSpInfoList();
											LotterySp sp = new LotterySp();
											for (int k = 0; k < sps.size(); k++) {
												MatchAgainstSpInfo spInfo = sps.get(k);
												if (spInfo.getPlayMethod().equals(GlobalConstants.TC_JZSPF) || spInfo.getPlayMethod().equals(GlobalConstants.TC_JZXSPF)) {
													if (spInfo.getPassType().equals("1")) {
														if (!matchList.contains(m)) {
															matchList.add(m);
															sp = FootballLotteryTools.getLotterySp(spInfo, sp, "1");
														}
													}
												}
											}
										...
										sps.add(tempSps);
										mSingleMatchs.add(matchList);
									}
									if (mAdapter != null) {
										mAdapter.setData(spiltTimes, mSingleMatchs, sps);
									}
									break;
								default:
									break;
		 ...
		 * @date:2015年11月16日
		 */
		public void getFootBallMatchsInfoSPFFailure(final String error) {
			if (mFragmentActivity != null) {
				mFragmentActivity.runOnUiThread(new Runnable() {
	...
	public void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if (requestCode == mlotoId) {
			if (data != null) {
				mAdapter.setSelectItemIndexs((Map<Integer, Map<Integer, List<Integer>>>) data.getSerializableExtra(LotteryBidCartActivity.LOTTERYBIDMATCH_SELECT_INDEXS));
				// 获取已经选择的场次集合，用于更新底部UI
				((LotteryFootballActivity) mFragmentActivity).changeSelectMatchCount2state(mAdapter.getSelectItemIndexs() == null ? 0 : mAdapter.getSelectItemIndexs().size());
			}
		}
