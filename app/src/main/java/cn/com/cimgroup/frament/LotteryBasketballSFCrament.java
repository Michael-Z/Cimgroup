...
import cn.com.cimgroup.adapter.LotteryBasketBallSFCAdapter;
 ...
 * @Date:2016-6-3
 */
public class LotteryBasketballSFCrament extends BaseLotteryBidFrament {

	List<String> spiltTimes = new ArrayList<String>();

	List<List<Match>> mMatchs = new ArrayList<List<Match>>();

	private int mlotoId = FootballLotteryConstants.L308;

	private LotteryBasketBallSFCAdapter mAdapter;
	...
	public void initData() {
		mAdapter = new LotteryBasketBallSFCAdapter(mFragmentActivity,
				mSearchType);
	...
	private CallBack mCallBack = new CallBack() {
		 ...
		 * 篮球胜分差加载成功
		 */
		public void getBasketballMatchsSFCSuccess(final FootBallMatch info) {
			// 将引用传递中得值重新赋值给变量
		 ...
		 * 篮球胜分差获取失败
		 */
		public void getBasketballMatchsSFCFailure(final String error) {
			if (mFragmentActivity != null) {
				mFragmentActivity.runOnUiThread(new Runnable() {
	...
	public void onSuperActivityOnClick(int id, Object... data) {
		...
		case R.id.txtView_lotterybid_match_finish:
			int matchCount = mAdapter.getSelectItemIndexs() == null ? 0
			...
			LotteryBidCartActivity.forwardLotteryBidOnlyCartActivity(
	...
	public void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if (requestCode == mlotoId) {
			if (data != null) {
				...
				mAdapter.setSelectItemIndexs((Map<Integer, List<Integer>>) data
	...
	protected void loadData(int page) {
		((LotteryBasketballActivity) getActivity()).addData(2);
	}
}
