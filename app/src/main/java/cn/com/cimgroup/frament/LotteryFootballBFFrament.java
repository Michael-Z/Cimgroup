...
import cn.com.cimgroup.adapter.LotteryFootBallBFAdapter;
...
/**
 * 足彩 比分 TODO足彩数据从LotteryFootballActivity中获取
 * @Description:
 * @author:zhangjf 
 * @copyright www.wenchuang.com
 * @Date:2016年1月25日
 */
public class LotteryFootballBFFrament extends BaseLotteryBidFrament {

	// 比分Adapter
	private LotteryFootBallBFAdapter mAdapter = null;
	
	List<String> spiltTimes = new ArrayList<String>();
	
	List<List<Match>> mMatchs = new ArrayList<List<Match>>();

	private int mlotoId = FootballLotteryConstants.L302;
	...
	public void initData() {
		mAdapter = new LotteryFootBallBFAdapter(mFragmentActivity, mSearchType);
	...
	private CallBack mCallBack = new CallBack() {
		 ...
		 * 足彩胜负彩加载成功
		 */
		public void getFootBallMatchsBFSuccess(final FootBallMatch info) {
			// 将引用传递中得值重新赋值给变量
		 ...
		 * 足彩胜负彩获取失败
		 */
		public void getFootBallMatchsBFFailure(final String error) {
			if (mFragmentActivity != null) {
				mFragmentActivity.runOnUiThread(new Runnable() {
	...
	protected void loadData(int page) {
		((LotteryFootballActivity)getActivity()).addData(1);
	}
