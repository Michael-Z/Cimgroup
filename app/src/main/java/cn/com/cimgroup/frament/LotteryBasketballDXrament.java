...
import cn.com.cimgroup.adapter.LotteryBasketBallDXAdapter;
 ...
 * @Date:2016-6-3
 */
public class LotteryBasketballDXrament extends BaseLotteryBidFrament {

	private LotteryBasketBallDXAdapter mAdapter;
	
	List<String> spiltTimes = new ArrayList<String>();
	
	List<List<Match>> mMatchs = new ArrayList<List<Match>>();

	private int mlotoId = FootballLotteryConstants.L309;
	...
	public void initData() {
		mAdapter = new LotteryBasketBallDXAdapter(mFragmentActivity, mSearchType);
	...
	private CallBack mCallBack = new CallBack() {
		 ...
		 * 篮球大小加载成功
		 */
		public void getBasketballMatchsDXSuccess(final FootBallMatch info) {
			// 将引用传递中得值重新赋值给变量
									...
									for (int i = 0; i < matchs.size(); i++) {
										...
										for (int j = 0; j < match.getMatchs().size(); j++) {
											...
											for (int k = 0; k < sps.size(); k++) {
												...
												if (spInfo.getPlayMethod().equals(GlobalConstants.TC_JLDXF)) {
		 ...
		 * 篮球大小获取失败
		 */
		public void getBasketballMatchsDXFailure(final String error) {
			if (mFragmentActivity != null) {
				mFragmentActivity.runOnUiThread(new Runnable() {
...
//				mAdapter.setData(spiltTimes, mMatchs, sps);
	...
	protected void loadData(int page) {
		((LotteryBasketballActivity)getActivity()).addData(1);
	}
}
